...
Only certain items that /require/ org need to be inside the ~with-eval-after-load~ part. So we've reorganised this a little.

#+begin_src emacs-lisp :exports none
  ;; Default directory for org files. This is set outside org loading to ensure it
  ;; is always available.
  (setq org-directory "~/github/org"
        ...
        org-journal-file (expand-file-name "journal-club.org" org-directory)
        ...
        general-templates `(("t" "Todo's and other general notes")
                            ...
                            ("tr" "Ticklers/Reminders" entry (file+headline ,(expand-file-name "general.org" org-directory) "Ticklers")
                             "* TODO %?\nCaptured: %u\n" :clock-in nil :jump-to-captured t :empty-lines 1)
                            ("j" "Journal Club" entry (file org-journal-file)
                             "* DETAILS: %? :JOURNAL CLUB: \n%u\n" :clock-in nil :jump-to-captured t :empty-lines 1)
                            ("P" "Professional development templates")
                            ("Ps" "Seminar" entry (file+headline ,(expand-file-name "professional-development.org" org-directory) "Seminars")
                             "* Title: %?\n** Date: %^u\n** Speaker: \n** Notes\n" :clock-in nil :jump-to-captured t :empty-lines 1)
                            ("Pw" "Workshop" entry (file+headline ,(expand-file-name "professional-development.org" org-directory) "Workshops")
                             "* Title: %?\n** Date: %^u\n** Presenter: " :clock-in nil :jump-to-captured t :empty-lines 1)
                            ("i" "Issue tracking" entry (file ,(expand-file-name "issue-tracking.org" org-directory))
                             "* TODO %^{Title}\n%^{Type}p\nCaptured: %u\n** Description\n" :clock-in nil :jump-to-captured t :empty-lines 1)
                            ("J" "Journal wins/losses" entry (file+function ,(expand-file-name "wins-and-losses.org" org-directory) sprazza/org-get-target-headline)
                             "* %u\n** %U\n%?" :clock-in nil :jump-to-captured t :empty-lines 1)
                            )
        )

  ;; Now combine the general with the specific for actual captures
  (load (expand-file-name "~/github/dot-emacs-extras/org-capture-extras.el") 'noerror 'nomessage)
  (setq org-capture-templates
        (append
         general-templates
         sprazza/extra-templates
         )
  ...
  ;; And here's some function definitions that are used (but don't require ord)
...
There's an issue with citar-capf apparently. Not entirely sure, but I'll try turning it off during capture and see what happens.
...
#+BEGIN_SRC emacs-lisp :exports none
  ...
  (with-eval-after-load 'org
    ...
    ;; Turn of citar completion in org-capture
