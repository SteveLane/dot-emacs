...
(defvar sphinx/python-split-arg-regex
  (concat
...
(defun sphinx/python-split-args (arg-string)
...
default values.  An argument named `self` is omitted."
  (remove
   ...
   (mapcar (lambda (x)           ; organize output
             (when (and
                    (not (equal "self" x))
                    (string-match sphinx/python-split-arg-regex x)
                    )
               (list
...
(defun sphinx/python-args-to-docstring ()
  "Return docstring format for the python arguments in yas-text."
  (let* ((indent (concat "\n" (make-string (current-column) 32)))
         (args (sphinx/python-split-args yas-text))
         (max-len (if args (apply 'max (mapcar (lambda (x) (length (nth 0 x))) args)) 0))
         (formatted-args (mapconcat
                          (lambda (x)
                            (concat ":param " (nth 0 x) (make-string (- max-len (length (nth 0 x))) ? ) ": "
                                    (if (nth 1 x) (concat "\n:type " (nth 0 x) ": " (nth 1 x)))
